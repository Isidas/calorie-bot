...
- **Добавлено:** уточняющий диалог после анализа блюда: бот может задать один дополнительный вопрос (например, наличие крема/сливок у десертов или масла/майонеза у салатов), чтобы повысить точность расчёта калорий и БЖУ.
- **Dialog state (in-memory):** введён слой хранения активного диалога пользователя в `src/services/dialog-state.ts`: тип `DialogState` (userId, baseAnalysis, question, startedAt), функции `setDialog`, `getDialog`, `clearDialog`. Состояние хранится в памяти (Map по userId); TTL в MVP не реализован.
- **Clarification service:** в `src/services/clarification-service.ts` реализованы: `shouldAskClarification(analysis)` — возвращает true, если confidence ≠ high или название блюда содержит ключевые слова (cake, dessert, salad, pasta, sandwich); `generateQuestion(analysis)` — для десертов/тортов возвращает вопрос «Есть ли крем или сливки?» (id: cream), для салатов — «Добавлено ли масло или майонез?» (id: sauce), с вариантами ответа yes/no; `applyCorrection(analysis, answer, questionId)` — применяет коэффициенты к калориям и жирам (cream+yes: ×1.25 калории, ×1.3 жиры; sauce+yes: ×1.2 калории, ×1.25 жиры) и возвращает новый `DishAnalysis`.
- **Интеграция в хендлеры:** после успешной отправки основного результата (редактирование сообщения «Анализирую…» на текст с нутриентами) выполняется проверка `shouldAskClarification`; при true и наличии сгенерированного вопроса бот отправляет **новое сообщение** с текстом вопроса и inline-кнопками «Да» / «Нет», сохраняет состояние диалога через `setDialog`.
- **Callback-обработчик:** добавлен обработчик действий с префиксом `clarify:{questionId}:{answer}`. При нажатии кнопки: из состояния по userId достаётся сохранённый `baseAnalysis`, вызывается `applyCorrection`, пользователю отправляется **новое сообщение** с пересчитанными нутриентами (тот же формат, что и основной результат, через `formatResult`), затем диалог очищается (`clearDialog`). Старое сообщение с результатом не редактируется — бот всегда отправляет новый ответ после уточнения.
- **Архитектура:** состояние уточняющего диалога хранится только в памяти (один активный диалог на пользователя). Пересчёт нутриентов выполняется в clarification-service по коэффициентам; исходный `DishAnalysis` не меняется в истории. Вопросы задаются только для блюд, по которым реализован конкретный вопрос (десерты/торты — крем, салаты — соус); при низкой уверенности или наличии в названии других ключевых слов (pasta, sandwich) уточнение пока не задаётся (generateQuestion возвращает null), чтобы не расширять сценарии без явного вопроса в продукте.
...
5. **bot/handlers.ts:** по **DishAnalysis** формируется текст (диапазон калорий, БЖУ, вес, 1–3 assumptions, дисклеймер), сообщение «Анализирую…» редактируется на результат. Если включён уточняющий диалог (см. Clarification Flow): при `shouldAskClarification(analysis)` и наличии вопроса (десерт/торт — крем, салат — соус) бот отправляет новое сообщение с вопросом и inline-кнопками «Да»/«Нет», сохраняет состояние диалога. При нажатии кнопки обрабатывается callback `clarify:{questionId}:{answer}`: из состояния берётся исходный анализ, применяется коррекция нутриентов, пользователю отправляется **новое сообщение** с пересчитанными калориями/БЖУ (старое не редактируется), диалог очищается. Ошибки: rate limit, USDA, vision, сеть — отдельные сообщения пользователю.
...
### 3.9. Dialog state (`src/services/dialog-state.ts`)

**Что сделано:** In-memory хранилище активного уточняющего диалога: тип **DialogState** (userId, baseAnalysis, question, startedAt), тип **ClarificationQuestion** (id, text, options). Функции **setDialog(userId, state)**, **getDialog(userId)**, **clearDialog(userId)**; внутри — Map по userId. TTL в MVP не используется.

**Зачем так:** Один активный диалог на пользователя; после ответа на кнопку состояние сбрасывается, повторное нажатие не пересчитывает результат (диалог уже очищен).

---

### 3.10. Clarification service (`src/services/clarification-service.ts`)

**Что сделано:** **shouldAskClarification(analysis):** true, если confidence ≠ high или в названии блюда (lowercase) встречается одно из: cake, dessert, salad, pasta, sandwich. **generateQuestion(analysis):** для блюд, содержащих "cake" или "dessert", — вопрос «Есть ли крем или сливки?» (id: cream, варианты yes/no); для "salad" — «Добавлено ли масло или майонез?» (id: sauce); для остальных ключевых слов вопрос не задаётся (null). **applyCorrection(analysis, answer, questionId):** при cream+yes — калории ×1.25, жиры ×1.3; при sauce+yes — калории ×1.2, жиры ×1.25; иначе возврат копии analysis без изменений; возвращается новый объект DishAnalysis (в т.ч. обновлённый calories_range).

**Зачем так:** Повышение точности для составных блюд без изменения контрактов vision/nutrition; пересчёт только на стороне бота по фиксированным коэффициентам.

---

### 3.11. Хендлеры бота (`src/bot/handlers.ts`)

**Что сделано:**
- **/start:** приветствие и описание (без изменений логики).
- **Фото:** получение буфера через `getFileBuffer`, вызов `dishService.analyzeFromImage`, редактирование «Анализирую…» на результат или на сообщение об ошибке. После успешного результата при `shouldAskClarification(analysis)` и наличии вопроса от `generateQuestion` — отправка нового сообщения с текстом вопроса и inline-кнопками (Да/Нет), сохранение состояния диалога (`setDialog`). Все вызовы к Telegram обёрнуты в **withTelegramRetry**. Внешний try/catch по всему телу хендлера — чтобы ни одна ошибка не уходила в «Unhandled».
- **Clarification callback:** обработчик действий `clarify:{questionId}:{answer}`: по userId загружается диалог (`getDialog`), применяется `applyCorrection`, отправляется **новое сообщение** с `formatResult(updated)` (старое не редактируется), вызывается `clearDialog`.
